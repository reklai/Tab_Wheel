...
test("content script implements modifier-wheel and left/middle/right click actions", () => {
  ...
  assert.match(source, /event\.type === "contextmenu" && activeClaim\.action === "closeToRecent"[\s\S]*runMouseGestureAction\(activeClaim\.action\)/);
  assert.match(source, /action !== "closeToRecent" \|\| event\.type === "contextmenu"/);
