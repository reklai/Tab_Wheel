...
test("domain supports MRU cycling, restricted-page skipping, and URL-validated scroll memory", () => {
  ...
  assert.match(source, /cycleUnlocked[\s\S]*cancelScrollRestore\(activeTab\.id\)[\s\S]*void captureTabScroll\(activeTab\)\.catch\(\(\) => \{\}\)[\s\S]*activateTab\(targetTab,\s*\{\s*restoreScrollAsync:\s*true\s*\}\)/);
  ...
  assert.match(source, /browser\.tabs\.onRemoved\.addListener[\s\S]*invalidateWindowTabsCache\(removeInfo\?\.windowId\)[\s\S]*scrollRestoreTokensByTabId\.delete\(tabId\)[\s\S]*activeTabIdsByWindowId[\s\S]*activeTabIdsByWindowId\.delete\(windowId\)/);
  assert.match(source, /browser\.tabs\.onUpdated\.addListener[\s\S]*invalidateWindowTabsCache\(updatedTab\?\.windowId\)[\s\S]*cancelScrollRestore\(tabId\)/);
  assert.match(source, /browser\.windows\.onRemoved\.addListener[\s\S]*activeTabIdsByWindowId\.delete\(windowId\)/);
  assert.match(source, /browser\.runtime\.onStartup\.addListener[\s\S]*windowTabsCacheByWindowId\.clear\(\)[\s\S]*scrollRestoreTokensByTabId\.clear\(\)[\s\S]*activeTabIdsByWindowId\.clear\(\)/);
