...
test("content script implements modifier-wheel and left/middle/right click actions", () => {
  ...
  assert.match(source, /settings\.openNativeNewTabOnLeftClick/);
...
test("settings contract exposes MRU scope, restricted-page skipping, and wheel tuning", () => {
  ...
  assert.match(contract, /openNativeNewTabOnLeftClick:\s*false/);
...
test("domain supports MRU cycling, restricted-page skipping, and URL-validated scroll memory", () => {
  ...
  assert.match(source, /resolveAvailableCycleTargetTab[\s\S]*ensurePageGestureAvailable\(targetTab\)/);
  assert.match(source, /resolveMostRecentAvailableTab[\s\S]*ensurePageGestureAvailable\(targetTab\)/);
  ...
  assert.match(source, /async function getWindowTabs\(windowId: number\): Promise<Tabs\.Tab\[]>[\s\S]*windowTabsCacheByWindowId\.get\(windowId\)[\s\S]*browser\.tabs\.query\(\{\s*windowId\s*\}\)[\s\S]*windowTabsCacheByWindowId\.set\(windowId/);
  ...
  assert.match(source, /restoreScroll\(tab: Tabs\.Tab\): Promise<boolean>[\s\S]*const restoreToken = beginScrollRestore\(tab\.id\)[\s\S]*for \(const delay of SCROLL_RESTORE_RETRY_DELAYS_MS\)[\s\S]*isScrollRestoreCurrent\(tab\.id,\s*restoreToken\)/);
  assert.match(source, /cycleUnlocked[\s\S]*resolveAvailableCycleTargetTab\(activeTab,\s*candidateTabs,\s*direction,\s*settings\)[\s\S]*cancelScrollRestore\(activeTab\.id\)[\s\S]*void captureTabScroll\(activeTab\)\.catch\(\(\) => \{\}\)[\s\S]*activateTab\(targetTab,\s*\{\s*restoreScrollAsync:\s*true\s*\}\)/);
  ...
  assert.match(source, /activateMostRecentTab[\s\S]*resolveMostRecentAvailableTab\(activeTab,\s*activeTab\.windowId,\s*eligibleTabs,\s*settings\)[\s\S]*void captureTabScroll\(activeTab\)\.catch\(\(\) => \{\}\)[\s\S]*activateTab\(targetTab,\s*\{\s*restoreScrollAsync:\s*true\s*\}\)/);
  ...
  assert.match(source, /browser\.runtime\.onStartup\.addListener[\s\S]*windowTabsCacheByWindowId\.clear\(\)[\s\S]*contentScriptUnavailableUrlsByTabId\.clear\(\)[\s\S]*scrollRestoreTokensByTabId\.clear\(\)[\s\S]*activeTabIdsByWindowId\.clear\(\)/);
...
test("popup exposes MRU mode and fallback controls", () => {
  ...
  assert.match(popupSource, /Left-click opens new tab\./);
  assert.match(popupSource, /Middle-click opens most recent tab\./);
  assert.match(popupSource, /Right-click closes current tab\./);
  ...
  assert.match(popupHtml, /Browser New Tab Page Mode/);
  assert.match(popupHtml, /id="leftClickActionLabel"/);
  assert.match(popupHtml, /id="leftClickSearchBtn"/);
  assert.match(popupHtml, /id="leftClickNativeBtn"/);
  assert.ok(
    popupHtml.indexOf('id="leftClickNativeBtn"') < popupHtml.indexOf('id="searchForm"'),
    "Expected left-click action pill to appear above the search form.",
  );
...
test("options and help document the MRU click gesture model", () => {
  ...
  const nativeNewTabIndex = optionsHtml.indexOf('for="openNativeNewTabOnLeftClick"');
  ...
  assert.ok(nativeNewTabIndex > modeIndex);
  assert.ok(presetIndex > nativeNewTabIndex);
  assert.ok(modifierIndex > presetIndex);
  assert.ok(shiftIndex > modifierIndex);
  assert.ok(invertIndex > shiftIndex);
  assert.ok(skipPinnedIndex > invertIndex);
  assert.ok(skipRestrictedIndex > skipPinnedIndex);
  assert.ok(wrapIndex > skipRestrictedIndex);
  assert.ok(accelerationIndex > wrapIndex);
  ...
  assert.match(optionsSource, /openNativeNewTabOnLeftClickSelect/);
  assert.match(optionsSource, /Most Recently Used/);
  assert.match(optionsSource, /Left-To-Right/);
  ...
  assert.match(helpSource, /opens the in-page search launcher/);
  assert.match(helpSource, /opens the browser's normal new tab page/);
  assert.match(helpSource, /token:\s*`\$\{gestureModifier\} \+ Middle Click`/);
  assert.match(helpSource, /jumps to the most recently used tab/);
  assert.match(helpSource, /token:\s*`\$\{gestureModifier\} \+ Right Click`/);
  assert.match(helpSource, /closes this tab/);
