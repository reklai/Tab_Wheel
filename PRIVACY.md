...
- **Scroll memory** (`tabWheelScrollMemory`) - recent tab IDs, window IDs, page URLs used only to validate scroll restore, root scroll X/Y positions, normalized page position, page and viewport dimensions, browser zoom, and update timestamps, bounded to 300 entries.
...
| `storage` | Save settings, fallback search URL template, MRU tab order, scroll positions, page geometry, tab zoom, and schema version locally |
