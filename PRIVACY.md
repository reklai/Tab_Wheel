...
TabWheel does not collect, transmit, or share user data with developer-owned services. Settings and extension state stay in your browser. Submitted TabWheel Search queries go to the browser's current default search provider, with the Google fallback used only if the browser search API is unavailable. The search launcher's as-you-type suggestions (your recent searches, and open tabs when you type `/tab`) are matched locally on your device and are never transmitted anywhere.
