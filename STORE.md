...
CLICK ACTIONS:
TabWheel Search opens the in-page search launcher. As you type, it suggests your recent searches and, when you type /tab, your open tabs, matched locally on your device with fuzzy highlighting and keyboard navigation; press Enter to run a web search or jump to the selected tab. Search uses the browser's default search provider first, with a fixed Google fallback if the browser search API is unavailable. Browser Default opens the browser's normal new tab page. Left, middle, and right click can each be remapped to Search, Browser Default, Most Recent Tab, Close Tab, Duplicate Tab, Open Settings, or native click pass-through.
